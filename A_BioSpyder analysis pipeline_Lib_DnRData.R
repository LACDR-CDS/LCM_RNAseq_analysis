...
PCAplots <- function(pca = df_pca,            #Variable of PCA results derived from PCADF function
  ...
  if(Dataclass == "log2"){
    Vari <- c("SAMPLE_ID",  "TREATMENT", "REPLICATE", "EXP_ID",  "LOCATION_ID", "Cluster", "CONCENTRATION")
    ID <- "SAMPLE_ID"
  }
  
  if(Dataclass == "log2FC"){
    Vari <- c("meanID", "CELL_ID", "TREATMENT", "CONCENTRATION", "TIMEPOINT", "EXP_ID", "dose_level")
    ID <- "meanID"
  }
